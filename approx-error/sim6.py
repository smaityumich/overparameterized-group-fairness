...
if not os.path.exists('subsampling_sigma/'):
    os.mkdir('subsampling_sigma/')
...
for SNR in SNRs:
    ...
    # with open(f'temp_sigma/mse_{iteration}_{SNR}_same.txt', 'w') as f:
    ...
    with open(f'subsampling_sigma/mse_{iteration}_{SNR}_diff.txt', 'w') as f:
        for nodes in nodes_list:
            train_mse, train_mse_bal, majority_mse, minority_mse = mse_overparameter(train_data, test_majority,\
                test_minority, nodes=int(nodes), weighted=False)
            output = {'optimization': 'ERM', 'nodes': nodes, 'SNR': SNR, 'train-mse':train_mse,\
